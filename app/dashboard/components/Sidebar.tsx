...
const Sidebar: React.FC<Props> = ({ children }) => {
  ...
  return (
    <div className="mt-20 flex h-full w-full items-start justify-start shadow-sm">
      {showSidebar ? (
        <aside
          ...
          className={classNames(
            showSidebar ? " fixed w-11/12 lg:w-fit lg:min-w-64" : "",
            " z-50 h-[calc(100vh-80px)]  border border-gray-200 bg-white",
          )}
          aria-label="Sidebar"
        >
      ...
      <div
        className={classNames(
          showSidebar ? "w-full lg:ml-auto lg:w-[calc(100vw-256px)]" : "w-full",
          " bg-gray-50 px-10",
        )}
      >
        {children}
